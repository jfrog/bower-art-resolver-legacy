                ...
                if (notFound) {
                    return ArtifactoryResolver.doArtifactoryRequest(requestUrl, that._config)
                        .then(function (response) {
                            ...
                            var err = createError(message, '404');
                            err.details = message;
                            throw err;
                        })
                        .fail(function (response) {
                            var err = createError(response.message, 'response');
...
ArtifactoryResolver.refs = function (source) {
    ...
    var requestUrl = ArtifactoryResolver.extractArtifactoryRegistry(config) + '/refs/' + pair.org + '/' + pair.repo;
